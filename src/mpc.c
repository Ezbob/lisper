...
static mpc_input_t *mpc_input_new_nstring(const char *filename, const char *string,
                                          size_t length) {
...
char *mpc_err_string(mpc_err_t *x) {
  ...
  mpc_err_string_cat(buffer, &pos, &max, "%s:%i:%i: error: expected ", x->filename,
  ...
  if (x->expected_num >= 2) {
    ...
    mpc_err_string_cat(buffer, &pos, &max, "%s or %s", x->expected[x->expected_num - 2],
...
static mpc_val_t *mpc_parse_apply_to(mpc_input_t *i, mpc_apply_to_t f, mpc_val_t *x,
                                     mpc_val_t *d) {
...
static int mpc_parse_run(mpc_input_t *i, mpc_parser_t *p, mpc_result_t *r,
  ...
  case MPC_TYPE_RANGE:
    MPC_PRIMITIVE(
        mpc_input_range(i, p->data.range.x, p->data.range.y, (char **)&r->output));
  case MPC_TYPE_ONEOF:
  ...
  case MPC_TYPE_APPLY_TO:
    if (mpc_parse_run(i, p->data.apply_to.x, r, e)) {
      MPC_SUCCESS(
          mpc_parse_apply_to(i, p->data.apply_to.f, r->output, p->data.apply_to.d));
    } else {
      MPC_FAILURE(r->error);
    }
  ...
  case MPC_TYPE_OR:
    ...
    MPC_FAILURE(NULL; if (p->data.or.n > MPC_PARSE_STACK_MIN) { mpc_free(i, results); });
  ...
  case MPC_TYPE_AND:
    ...
    mpc_input_unmark(i);
    MPC_SUCCESS(mpc_parse_fold(i, p->data.and.f, j, (mpc_val_t **)results);
                if (p->data.or.n > MPC_PARSE_STACK_MIN) { mpc_free(i, results); });
...
int mpc_nparse(const char *filename, const char *string, size_t length, mpc_parser_t *p,
               mpc_result_t *r) {
  int x;
...
mpc_parser_t *mpc_maybe(mpc_parser_t *a) { return mpc_maybe_lift(a, mpcf_ctor_null); }
...
mpc_parser_t *mpc_alpha(void) {
  return mpc_expect(mpc_oneof("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ"),
                    "letter");
}
...
mpc_parser_t *mpc_real(void) {
  ...
  p1 = mpc_digits();
  p2 = mpc_maybe_lift(mpc_and(2, mpcf_strfold, mpc_char('.'), mpc_digits(), free),
                      mpcf_ctor_str);
  p30 = mpc_oneof("eE");
...
mpc_parser_t *mpc_char_lit(void) {
  return mpc_expect(mpc_between(mpc_or(2, mpc_escape(), mpc_any()), free, "'", "'"),
                    "char");
}

mpc_parser_t *mpc_string_lit(void) {
  mpc_parser_t *strchar = mpc_or(2, mpc_escape(), mpc_noneof("\""));
  return mpc_expect(mpc_between(mpc_many(mpcf_strfold, strchar), free, "\"", "\""),
                    "string");
}

mpc_parser_t *mpc_regex_lit(void) {
  mpc_parser_t *regexchar = mpc_or(2, mpc_escape(), mpc_noneof("/"));
  return mpc_expect(mpc_between(mpc_many(mpcf_strfold, regexchar), free, "/", "/"),
                    "regex");
}
...
mpc_parser_t *mpc_tok_between(mpc_parser_t *a, mpc_dtor_t ad, const char *o,
                              const char *c) {
  return mpc_and(3, mpcf_snd_free, mpc_sym(o), mpc_tok(a), mpc_sym(c), free, ad);
}
...
mpc_parser_t *mpc_re(const char *re) {
  ...
  mpc_define(Factor, mpc_and(2, mpcf_re_repeat, Base,
...
mpc_val_t *mpcf_strtrim(mpc_val_t *x) { return mpcf_strtriml(mpcf_strtrimr(x)); }
...
mpc_val_t *mpcf_unescape_char_raw(mpc_val_t *x) {
  mpc_val_t *y =
      mpcf_unescape_new(x, mpc_escape_input_raw_cchar, mpc_escape_output_raw_cchar);
  free(x);
  return y;
}
...
static void mpc_print_unretained(mpc_parser_t *p, int force) {
  ...
  if (p->type == MPC_TYPE_ONEOF) {
    s = mpcf_escape_new(p->data.string.x, mpc_escape_input_c, mpc_escape_output_c);
    printf("[%s]", s);
    free(s);
  }
...
static void mpc_ast_print_depth(mpc_ast_t *a, int d, FILE *fp) {
  ...
  if (strlen(a->contents)) {
    fprintf(fp, "%s:%lu:%lu '%s'\n", a->tag, (long unsigned int)(a->state.row + 1),
...
mpc_ast_t *mpc_ast_traverse_next(mpc_ast_trav_t **trav) {
  ...
  case mpc_ast_trav_order_pre:
    ...
    while (*trav != NULL && (*trav)->curr_child >= (*trav)->curr_node->children_num) {
      to_free = *trav;
      *trav = (*trav)->parent;
      free(to_free);
    }
...
static mpc_val_t *mpcaf_grammar_regex(mpc_val_t *x, void *s) {
  ...
  mpc_parser_t *p =
      (st->flags & MPCA_LANG_WHITESPACE_SENSITIVE) ? mpc_re(y) : mpc_tok(mpc_re(y));
  free(y);
  ...
  if (is_number(x)) {
    ...
    while (st->parsers_num <= i) {
      ...
      if (st->parsers[st->parsers_num - 1] == NULL) {
        return mpc_failf("No Parser in position %i! Only supplied %i Parsers!", i,
                         st->parsers_num);
      }
    }
...
mpc_parser_t *mpca_grammar_st(const char *grammar, mpca_grammar_st_t *st) {
  ...
  mpc_define(
      Base,
      mpc_or(5, mpc_apply_to(mpc_tok(mpc_string_lit()), mpcaf_grammar_string, st),
...
static mpc_err_t *mpca_lang_st(mpc_input_t *i, mpca_grammar_st_t *st) {
  ...
  mpc_define(Lang, mpc_apply_to(mpc_total(mpc_predictive(mpc_many(mpca_stmt_fold, Stmt)),
...
static void mpc_optimise_unretained(mpc_parser_t *p, int force) {
  ...
  while (1) {
    ...
    /* Merge lhs `or` */
    if (p->type == MPC_TYPE_OR && p->data.or.xs[0]->type == MPC_TYPE_OR && !p->data.or
                                                .xs[0]
                                                ->retained) {
      t = p->data.or.xs[0];
      n = p->data.or.n;
    ...
    if (p->type == MPC_TYPE_AND && p->data.and.n == 2 &&
        p->data.and.xs[0]->type == MPC_TYPE_PASS && !p->data.and.xs[0]->retained &&
        p->data.and.f == mpcf_fold_ast) {
      t = p->data.and.xs[1];
    ...
    if (p->type == MPC_TYPE_AND && p->data.and.f == mpcf_fold_ast &&
        p->data.and.xs[0]->type == MPC_TYPE_AND && !p->data.and.xs[0]->retained &&
