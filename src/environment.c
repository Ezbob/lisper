...
void lenvironment_add_builtin(struct lenvironment *e, char *name,
                              struct lvalue *(*func)(struct lenvironment *,
                                                     struct lvalue *)) {
...
void lenvironment_pretty_print(struct lenvironment *e) {
  for (size_t i = 0; i < e->capacity; ++i) {
    if (e->entries[i] != NULL) {
      printf("i: %zu    (n: '%s' t: '%s' p: %p)", i, e->entries[i]->name,
             ltype_name(e->entries[i]->envval->type), (void *)(e->entries[i]->envval));
