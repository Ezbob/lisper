...
#define LASSERT(lval, cond, fmt, ...) \
    if ( !(cond) ) { struct lvalue *err = lvalue_err(fmt, ##__VA_ARGS__); lvalue_del(lval); return err;  }
...
#define LNUM_ARGS(lval, func_name, numargs) \
    LASSERT(lval, lval->val.l.count == numargs, "Wrong number of arguments parsed to '%s'. Expected at exactly %lu argument(s); got %lu. ", func_name, numargs, lval->val.l.count)

#define LNOT_EMPTY_QEXPR(lval, func_name, i) \
    LASSERT(lval, lval->val.l.cells[i]->type == LVAL_QEXPR && lval->val.l.cells[i]->val.l.count > 0, "Empty %s parsed to '%s'.", ltype_name(lval->val.l.cells[i]->type, func_name))

#define LARG_TYPE(lval, func_name, i, expected) \
    LASSERT(lval, lval->val.l.cells[i]->type == expected, "Wrong type of argument parsed to '%s' at argument position %lu. Expected argument to be of type '%s'; got '%s'.", func_name, (i + 1), ltype_name(expected), ltype_name(lval->val.l.cells[i]->type))

#define LTWO_ARG_TYPES(lval, func_name, i, first_expect, second_expect) \
    LASSERT(lval, lval->val.l.cells[i]->type == first_expect || lval->val.l.cells[i]->type == second_expect, "Wrong type of argument parsed to '%s'. Expected argument to be of type '%s' or '%s'; got '%s'.", func_name, ltype_name(first_expect), ltype_name(second_expect), ltype_name(lval->val.l.cells[i]->type))
...
#define LMATH_TYPE_CHECK(lval, sym) do { \
    enum ltype expected_arg_type = LGETCELL(lval, 0)->type; \
    LASSERT(lval, LIS_NUM(expected_arg_type), "Cannot operate on argument at position %i. Non-number type '%s' parsed to operator '%s'.", 1, ltype_name(expected_arg_type), sym); \
    for ( size_t i = 1; i < lval->val.l.count; i++ ) { \
        struct lvalue *curr = LGETCELL(lval, i); \
        LASSERT(lval, expected_arg_type == curr->type, "Argument type mismatch. Expected argument at position %lu to be of type '%s'; got type '%s'.", i + 1, ltype_name(expected_arg_type), ltype_name(curr->type)); \
    } \
} while (0)
...
struct lvalue *builtin_open(struct lenvironment *e, struct lvalue *v) {
    ...
    if ( !(strcmp(m, "r") == 0 ||
           ...
           strcmp(m, "a") == 0 ||
           strcmp(m, "a+") == 0) ) {
        struct lvalue *err = lvalue_err("Mode not set to either 'r', 'r+', 'w', 'w+', 'a' or 'a+'");
        lvalue_del(mode);
        lvalue_del(filename);
        lvalue_del(v);
        return err;
