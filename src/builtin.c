...
#define LNUM_LEAST_ARGS(mp, sym, funcname, numargs)                                      \
  LASSERT(mp, sym, sym->val.l.count >= numargs,                                          \
...
#define LNUM_ARGS(mp, lval, func_name, numargs)                                          \
  LASSERT(mp, lval, lval->val.l.count == numargs,                                        \
...
#define LARG_TYPE(mp, lval, func_name, i, expected)                                      \
  LASSERT(mp, lval, lval->val.l.cells[i]->type == expected,                              \
...
struct lvalue *builtin_add(struct linterpreter *intp, struct lvalue *in) {
  do {
    enum ltype expected_arg_type = in->val.l.cells[0]->type;
    ...
    for (size_t i = 1; i < in->val.l.count; i++) {
      struct lvalue *curr = in->val.l.cells[i];
  ...
  if (collection->type == LVAL_QEXPR) {
    if (collection->val.l.count > 0) {
      lvalue_del(&intp->lvalue_mp,
                 lvalue_pop(&intp->lvalue_mp, collection, (collection->val.l.count - 1)));
    }
  } else {
...
struct lvalue *builtin_lambda(struct linterpreter *intp, struct lvalue *v) {
  ...
  for (size_t i = 0; i < formals->val.l.count; ++i) {
    LASSERT(&intp->lvalue_mp, v, formals->val.l.cells[i]->type == LVAL_SYM,
...
struct lvalue *builtin_fn(struct linterpreter *intp, struct lvalue *v) {
  ...
  LASSERT(&intp->lvalue_mp, v, name->val.l.count == 1,
...
struct lvalue *builtin_var(struct linterpreter *intp, struct lvalue *v, char *sym) {
  ...
  LASSERT(&intp->lvalue_mp, v, names->val.l.count == v->val.l.count - 1,
